...
const { industryneed, industry, year } = require('../models');

exports.getIndustryNeeds = async (req, res, next) => {
  try {
    if (!req.body) {
      req.body = {};
    }
    ...
    const data = await industryneed.findAll({
      where: req.body,
      include: [
        // Include is join
        {
    ...
    if (data.length === 0) {
      return next({
        message: 'Industry needs not found',
        statusCode: 404,
      });
    }
